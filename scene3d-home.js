...
// ═══════════════════════════════════════════════════════════════
// LOGO TEXTURE — Society logo as 3D billboard
// ═══════════════════════════════════════════════════════════════
const textureLoader = new THREE.TextureLoader();
textureLoader.load('logo-society.png', (texture) => {
  texture.colorSpace = THREE.SRGBColorSpace;
  const logoPlaneGeo = new THREE.PlaneGeometry(6, 6);
  ...
  logoPlane.position.set(8, 3, -5);
  logoPlane.userData = { baseY: 3 };
  scene.add(logoPlane);

  // Animate logo plane
  function animateLogoPlane() {
    requestAnimationFrame(animateLogoPlane);
    const time = clock.getElapsedTime();
    logoPlane.position.y = logoPlane.userData.baseY + Math.sin(time * 0.8) * 0.3;
    logoPlane.rotation.y = Math.sin(time * 0.3) * 0.2;
...
// ═══════════════════════════════════════════════════════════════
// MASCOT TEXTURES — Cipher & Ciphra billboards
// ═══════════════════════════════════════════════════════════════
textureLoader.load('cipher.png', (texture) => {
  texture.colorSpace = THREE.SRGBColorSpace;
  ...
  plane.position.set(-8, 2, -5);
  scene.add(plane);
  function anim() {
    requestAnimationFrame(anim);
    plane.lookAt(camera.position);
    plane.position.y = 2 + Math.sin(clock.getElapsedTime() * 0.8) * 0.2;
  }
  anim();
});

textureLoader.load('ciphra.png', (texture) => {
  texture.colorSpace = THREE.SRGBColorSpace;
  const mat = new THREE.MeshStandardMaterial({
    map: texture, transparent: true, side: THREE.DoubleSide,
    emissive: 0xffffff, emissiveIntensity: 0.05
  ...
  function anim() {
    requestAnimationFrame(anim);
    plane.lookAt(camera.position);
    plane.position.y = 2 + Math.sin(clock.getElapsedTime() * 0.8 + 1) * 0.2;
  }
  anim();
});
