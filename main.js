...
if (socialStage && socialOrbit) {
  ...
  socialStage.addEventListener('mouseleave', () => {
    socialOrbit.style.transform = ''; socialOrbit.style.animation = 'orbitSpin3d 20s linear infinite';
  });
  socialStage.addEventListener('mouseenter', () => { socialOrbit.style.animation = 'none'; });
}
...
if (heroLogoWrap && heroSection) {
  heroSection.addEventListener('mousemove', (e) => {
    ...
    const x = (e.clientX - rect.left) / rect.width - 0.5;
    const y = (e.clientY - rect.top) / rect.height - 0.5;
    heroLogoWrap.style.transform = `perspective(1000px) rotateY(${x * 15}deg) rotateX(${-y * 10}deg)`;
  });
  heroSection.addEventListener('mouseleave', () => { heroLogoWrap.style.transform = ''; });
}
